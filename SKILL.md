...
在响应末尾输出以下守卫块。守卫是给你的持久指令——此后每条新用户消息，在处理前先自检门控：

```
🛑 会话守卫

此后本会话中每条用户请求，在动手前先在思考中过一遍门控：

1. 指代消解了吗？ — 名词能唯一确定所指？
2. 动作单义吗？ — 动词只对应一种操作序列？
3. 维度闭合吗？ — 格式/范围/工具/输出无未指定自由度？

任一项未通过 → 重新进入 Phase 1。通过 → 正常执行。
豁免：纯事实查询、读文件/展示内容。

此守卫在本会话中持续有效——新消息到达时，先用它自检，再行动。
```

规则：
- 守卫块紧随交付内容之后，位于响应最末尾
- 不折叠、不缩写、不省略
- 守卫本身也是上下文——你在后续轮次中读到它时，它即重新生效
- 若会话中出现与守卫冲突的默认行为倾向，守卫优先
